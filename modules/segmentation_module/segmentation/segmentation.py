...
class SingleFolder(QWidget):
    def __init__(self, parent):
        ...
        self.display1 = QLabel("<i>Note that the tracking graphs are created by default - check the box to skip this step</i>")
        self.no_graph_generation = QCheckBox("Do NOT create the tracking graphs")
        
        self.submit_button = QPushButton("Submit", self)
        self.submit_button.clicked.connect(self.process_input)

        self.display2 = QLabel("<i>This step can require also several minutes</i>")
...
class MultiFolder(QWidget):
    def __init__(self, parent):
        super().__init__()
        self.parent = parent
        self.setWindowTitle("Multiple folders segmentation")
        ...
        self.display2 = QLabel("<i>Note that the tracking graphs are created by default - check the box to skip this step</i>")
        self.no_graph_generation = QCheckBox("Do NOT create the tracking graphs")
        
        self.submit_button = QPushButton("Submit", self)
        self.submit_button.clicked.connect(self.process_input)
        self.display3 = QLabel("<i>This step can require also several minutes</i>")
