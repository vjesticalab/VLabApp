...
Min area
: Remove mask regions with area (number of pixels) below this value.
...
: Minimum overlap fraction (w.r.t mask area) to consider when creating edges in the cell tracking graph. See [Cell tracking](#cell-tracking) in the appendix for more information.
...
* **Labels**: An integer ID (mask id) assigned to each segmented object (in this document, labels are represented by colors).
